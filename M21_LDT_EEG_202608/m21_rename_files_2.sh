...
# renames files with no taskID to add a task ID like "LDT"

# Find all files in subdirectories matching the specified pattern and rename them
find . -type f \( -name "S*.eeg" -o -name "S*.vhdr" -o -name "S*.vmrk" \) | while read -r file; do
  # Extract the subject identifier (e.g., "S101" from "S101.eeg")
  subject_id=$(echo "$file" | sed -E 's/.*(S[0-9]+)\..*/\1/')
